...
{
	bool bFound = false;
	char mapFound[PLATFORM_MAX_PATH];
	for (int i = 0; i < g_MapListTier.Length; i++)
	{
		GetArrayString(g_MapList, i, mapFound, sizeof(mapFound));
		if (StrEqual(mapFound, szMapName))
		{
			bFound = true;
			GetArrayString(g_MapListTier, i, szBuffer, sizeof(szBuffer));
			break;
		}
	}
